...
(function (MapMode) {
  ...
  /// was deleted.
  MapMode[MapMode["Simple"] = 0] = "Simple"; /// Return null if deletion happens across the position.

  MapMode[MapMode["TrackDel"] = 1] = "TrackDel"; /// Return null if the character _before_ the position is deleted.

  MapMode[MapMode["TrackBefore"] = 2] = "TrackBefore"; /// Return null if the character _after_ the position is deleted.
...
class SelectionRange {
  ...
  map(change) {
    let from = change.mapPos(this.from),
        to = change.mapPos(this.to);
...
let nextID = 0; /// A facet is a labeled value that is associated with an editor
...
class Facet {
  ...
  extensions) {
    ...
    this.extensions = extensions; /// @internal
...
const transactionExtender = Facet.define(); /// Annotations are tagged values that are used to add metadata to
...
(function (CharCategory) {
  /// Word characters.
  CharCategory[CharCategory["Word"] = 0] = "Word"; /// Whitespace.

  CharCategory[CharCategory["Space"] = 1] = "Space"; /// Anything else.
...
class EditorState {
  ...
  toJSON(fields) {
    ...
    if (fields) for (let prop in fields) result[prop] = fields[prop].spec.toJSON(this.field(fields[prop]), this);
    return result;
...
EditorState.allowMultipleSelections = allowMultipleSelections; /// Configures the tab size to use in this state. The first
...
EditorState.lineSeparator = lineSeparator; /// Registers translation phrases. The
/// [`phrase`](#state.EditorState.phrase) method will look through
/// all objects registered with this facet to find translations for
/// its argument.

EditorState.phrases = Facet.define(); /// A facet used to register [language
/// data](#state.EditorState.languageDataAt) providers.

EditorState.languageData = languageData; /// Facet used to register change filters, which are called for each
...
// (min-width: 400px)": {...}}`.
},{"process":"../node_modules/process/browser.js"}],"../node_modules/@codemirror/rangeset/dist/index.js":[function(require,module,exports) {
"use strict";
...
function textCoords(text, pos, side) {
  ...
  let rects = range.getClientRects(),
      rect = rects[(flatten ? flatten < 0 : side >= 0) ? 0 : rects.length - 1];
...
const BidiRE = /[\u0590-\u05f4\u0600-\u06ff\u0700-\u08ac]/; /// Represents a contiguous range of text that has a single direction
  ...
  if (linewise) {
    changes = {
      changes: state.selection.ranges.map(r => state.doc.lineAt(r.from)).filter((l, i, a) => i == 0 || a[i - 1] != l).map(line => ({
        from: line.from,
        insert: (byLine ? text.line(i++).text : input) + state.lineBreak
      }))
    };
  } else if (byLine) {
...
class DOMObserver {
  constructor(view, onChange, onScrollChanged) {
    ...
    this.onSelectionChange = event => {
      ...
      if (browser.ie && browser.ie_version <= 11 && !this.view.state.selection.main.empty && sel.focusNode && isEquivalentPosition(sel.focusNode, sel.focusOffset, sel.anchorNode, sel.anchorOffset)) this.flushSoon();else this.flush();
    };
...
const keymap = _state.Facet.define({
  enables: handleKeyEvents
});

exports.keymap = keymap;
const Keymaps = new WeakMap(); // This is hidden behind an indirection, rather than directly computed
...
const selectionConfig = _state.Facet.define({
  combine(configs) {
...
function measureRange(view, range) {
  ...
  function drawForLine(from, to) {
    ...
    function addSpan(from, fromOpen, to, toOpen, dir) {
      let fromCoords = view.coordsAtPos(from, 1),
          toCoords = view.coordsAtPos(to, -1);
    ...
    let start = from !== null && from !== void 0 ? from : view.moveToLineBoundary(_state.EditorSelection.cursor(to, 1), false).head;
    let end = to !== null && to !== void 0 ? to : view.moveToLineBoundary(_state.EditorSelection.cursor(from, -1), true).head; // Split the range by visible range and document line
...
const fromHistory = _state.Annotation.define(); /// Transaction annotation that will prevent that transaction from
...
function history(config = {}) {
  return [historyField, historyConfig.of(config), _view.EditorView.domEventHandlers({
    beforeinput(e, view) {
...
},{"@codemirror/state":"../node_modules/@codemirror/state/dist/index.js","@codemirror/view":"../node_modules/@codemirror/view/dist/index.js"}],"../node_modules/lezer-tree/dist/tree.es.js":[function(require,module,exports) {
"use strict";
...
exports.Tree = Tree;
Tree.empty = new Tree(NodeType.none, [], [], 0); /// Tree buffers contain (type, start, end, endIndex) quads for each
...
function buildTree(data) {
  ...
  function takeNode(parentStart, minPos, children, positions, inRepeat) {
    ...
    if (end - start <= maxBufferLength && (buffer = findBufferSize(cursor.pos - minPos, inRepeat))) {
      ...
      if (localInRepeat > -1 && localChildren.length > BalanceBranchFactor) node = balanceRange(type, type, localChildren, localPositions, 0, localChildren.length, 0, maxBufferLength, end - start);else node = new Tree(type, localChildren, localPositions, end - start);
    }
...
exports.languageDataProp = exports.language = exports.indentUnit = exports.indentService = exports.indentNodeProp = exports.foldService = exports.foldNodeProp = exports.flatIndent = exports.TreeIndentContext = exports.LezerLanguage = exports.LanguageSupport = exports.LanguageDescription = exports.Language = exports.IndentContext = exports.EditorParseContext = void 0;
...
const languageDataProp = new _lezerTree.NodeProp(); /// Helper function to define a facet (to be added to the top syntax
...
class Language {
  ...
  data, parser, extraExtensions = []) {
    ...
    this.parser = parser;
    this.extension = [language.of(this), _state.EditorState.languageData.of((state, pos) => state.facet(languageDataFacetAt(state, pos)))].concat(extraExtensions);
  ...
  parseString(code) {
    let doc = _text.Text.of(code.split("\n"));

    let parse = this.parser.startParse(new DocInput(doc), 0, new EditorParseContext(this.parser, _state.EditorState.create({
      doc
    }), [], _lezerTree.Tree.empty, {
      from: 0,
      to: code.length
    }, []));
    let tree;
...
Language.setState = _state.StateEffect.define();
...
function syntaxTree(state) {
  ...
  return field ? field.tree : _lezerTree.Tree.empty;
...
class EditorParseContext {
  /// @internal
  constructor(parser, /// The current editor state.
  state, /// Tree fragments that can be reused by incremental re-parses.
  fragments = [], /// @internal
  tree, /// The current editor viewport (or some overapproximation
  ...
  work(time, upto) {
    if (this.tree != _lezerTree.Tree.empty && (upto == null ? this.tree.length == this.state.doc.length : this.tree.length >= upto)) {
      this.takeTree();
      return true;
    }
    ...
    for (;;) {
      ...
      if (done) {
        this.fragments = this.withoutTempSkipped(_lezerTree.TreeFragment.addTree(done));
        this.parse = null;
        this.tree = done;
        return true;
  ...
  takeTree() {
    if (this.parse && this.parse.pos > this.tree.length) {
      ...
      this.fragments = this.withoutTempSkipped(_lezerTree.TreeFragment.addTree(this.tree, this.fragments, true));
    }
  }
  ...
  changes(changes, newState) {
    ...
    if (!changes.empty) {
      ...
      fragments = _lezerTree.TreeFragment.applyChanges(fragments, ranges);
      tree = _lezerTree.Tree.empty;
      viewport = {
  ...
  startParse(input, startPos, context) {
    return {
      ...
      advance() {
        ...
        return new _lezerTree.Tree(_lezerTree.NodeType.none, [], [], input.length - startPos);
      },
...
function cutFragments(fragments, from, to) {
  return _lezerTree.TreeFragment.applyChanges(fragments, [{
    fromA: from,
    toA: to,
    fromB: from,
    toB: to
...
class LanguageState {
  ...
  static init(state) {
    let parseState = new EditorParseContext(state.facet(language).parser, state, [], _lezerTree.Tree.empty, {
      from: 0,
      to: state.doc.length
    }, []);
...
Language.state = _state.StateField.define({
...
const parseWorker = _view.ViewPlugin.fromClass(class ParseWorker {
  constructor(view) {
    this.view = view;
...
const language = _state.Facet.define({
  combine(languages) {
...
class LanguageSupport {
  ...
  constructor( /// The language object.
  language, /// An optional set of supporting extensions. When nesting a
...
class LanguageDescription {
  constructor( /// The name of this language.
  name, /// Alternative names for the mode (lowercased, includes `this.name`).
  alias, /// File extensions associated with this language.
  extensions, /// Optional filename pattern that should be associated with this
  /// language.
  filename, loadFunc) {
    this.name = name;
    this.alias = alias;
    ...
    this.loadFunc = loadFunc; /// If the language has been loaded, this will hold its value.
...
const indentService = _state.Facet.define(); /// Facet for overriding the unit by which indentation happens.
...
const indentUnit = _state.Facet.define({
  combine: values => {
...
const indentNodeProp = new _lezerTree.NodeProp(); // Compute the indentation for a given position from the syntax tree.
...
class TreeIndentContext extends IndentContext {
  ...
  pos, /// The syntax tree node to which the indentation strategy
  /// applies.
  node) {
...
const foldService = _state.Facet.define(); /// This node prop is used to associate folding information with
...
},{"lezer-tree":"../node_modules/lezer-tree/dist/tree.es.js","@codemirror/text":"../node_modules/@codemirror/text/dist/index.js","@codemirror/state":"../node_modules/@codemirror/state/dist/index.js","@codemirror/view":"../node_modules/@codemirror/view/dist/index.js"}],"../node_modules/@codemirror/gutter/dist/index.js":[function(require,module,exports) {
"use strict";
...
const activeGutters = _state.Facet.define(); /// Define an editor gutter. The order in which the gutters appear is
...
const foldState = _state.StateField.define({
  ...
  provide: f => _view.EditorView.decorations.compute([f], s => s.field(f))
});
...
const foldWidget = _view.Decoration.replace({
...
function foldGutter(config = {}) {
  ...
  let markers = _view.ViewPlugin.fromClass(class {
    constructor(view) {
      ...
      this.markers = _rangeset.RangeSet.of(this.buildMarkers(view));
    }
    ...
    update(update) {
      ...
      if (!foldChange && update.docChanged && update.view.viewport.from == this.from && firstChange > this.from) {
        ...
        this.markers = _rangeset.RangeSet.of(this.buildMarkers(update.view));
      }
    }
...
function moveBySyntax(state, start, forward) {
  let pos = (0, _language.syntaxTree)(state).resolve(start.head);
...
const selectParentSyntax = ({
  ...
  let selection = updateSel(state.selection, range => {
    ...
    let context = (0, _language.syntaxTree)(state).resolve(range.head, 1);
...
const deleteByChar = (view, forward, codePoint) => deleteBy(view, pos => {
  ...
  if (codePoint) {
    ...
    target = (0, _text.findClusterBreak)(line.text, pos - line.from, forward) + line.from;
  }
...
const deleteByGroup = (view, forward) => deleteBy(view, pos => {
  ...
  for (let cat = null;;) {
    ...
    if (pos == (forward ? line.to : line.from)) {
      ...
      next = (0, _text.findClusterBreak)(line.text, pos - line.from, forward) + line.from;
...
function isBetweenBrackets(state, pos) {
  ...
  let context = (0, _language.syntaxTree)(state).resolve(pos);
  let before = context.childBefore(pos),
      after = context.childAfter(pos),
      closedBy;
  if (before && after && before.to <= pos && after.from >= pos && (closedBy = before.type.prop(_lezerTree.NodeProp.closedBy)) && closedBy.indexOf(after.name) > -1 && state.doc.lineAt(before.to).from == state.doc.lineAt(after.from).from) return {
    from: before.to,
    to: after.from
  };
  return null;
...
const insertNewlineAndIndent = ({
  ...
  let changes = state.changeByRange(({
    ...
    if (explode) insert.push((0, _language.indentString)(state, cx.lineIndent(line)));
    return {
      changes: {
        from,
        to,
...
}].concat(standardKeymap); /// A binding that binds Tab to [`insertTab`](#commands.insertTab) and
...
const closeBracketEffect = _state.StateEffect.define({
  map(value, mapping) {
...
const skipBracketEffect = _state.StateEffect.define({
  map(value, mapping) {
...
const closedBracket = new class extends _rangeset.RangeValue {}();
closedBracket.startSide = 1;
closedBracket.endSide = -1;

const bracketState = _state.StateField.define({
  create() {
...
function nodeStart(state, pos) {
  let tree = (0, _language.syntaxTree)(state).resolve(pos + 1);
...
exports.selectSelectionMatches = exports.selectMatches = exports.searchKeymap = exports.replaceNext = exports.replaceAll = exports.openSearchPanel = exports.gotoLine = exports.findPrevious = exports.findNext = exports.closeSearchPanel = exports.SearchCursor = void 0;
...
const dialogEffect = _state.StateEffect.define();

const dialogField = _state.StateField.define({
  create() {
    return true;
  },
  ...
  provide: f => _panel.showPanel.computeN([f], s => s.field(f) ? [createLineDialog] : [])
...
const gotoLine = view => {
  ...
  if (!panel) {
    view.dispatch({
      reconfigure: view.state.field(dialogField, false) == null ? {
        append: [(0, _panel.panels)(), dialogField, baseTheme]
      } : undefined,
...
function wordAt(doc, pos, check) {
  ...
  while (from > 0) {
    let prev = (0, _text.findClusterBreak)(line.text, from, false);
    if (check(line.text.slice(prev, from)) != _state.CharCategory.Word) break;
    from = prev;
  }

  while (to < line.length) {
    let next = (0, _text.findClusterBreak)(line.text, to);
    if (check(line.text.slice(to, next)) != _state.CharCategory.Word) break;
    to = next;
  }
...
const searchState = _state.StateField.define({
  ...
  provide: f => _panel.showPanel.computeN([f], s => s.field(f).panel)
});
...
const replaceNext = searchCommand((view, {
  ...
  let {
    state
      ...
      next = findNextMatch(state.doc, state.selection.main.from, query);
...
function buildPanel(conf) {
  ...
  let panel = (0, _crelt.default)("div", {
    onkeydown: keydown
  }, [searchField, button("next", () => findNext(conf.view), [p("next")]), button("prev", () => findPrevious(conf.view), [p("previous")]), button("select", () => selectMatches(conf.view), [p("all")]), (0, _crelt.default)("label", null, [caseField, "match case"]), (0, _crelt.default)("br"), replaceField, button("replace", () => replaceNext(conf.view), [p("replace")]), button("replaceAll", () => replaceAll(conf.view), [p("replace all")]), (0, _crelt.default)("button", {
    name: "close",
    onclick: () => closeSearchPanel(conf.view),
    "aria-label": p("close")
  }, ["×"]), (0, _crelt.default)("div", {
...
const baseTheme$1 = _view.EditorView.baseTheme({
  "$panel.search": {
    padding: "2px 6px 4px",
    position: "relative",
    "& [name=close]": {
  ...
  "$$light $searchMatch.selected": {
    backgroundColor: "#ff6a0054"
  },
  "$$dark $searchMatch.selected": {
    backgroundColor: "#ff00ff8a"
  }
});

const searchExtensions = [searchState, _state.Prec.override(searchHighlighter), (0, _panel.panels)(), baseTheme$1];
...
class CompletionContext {
  ...
  tokenBefore(types) {
    let token = (0, _language.syntaxTree)(this.state).resolve(this.pos, -1);
...
function ifNotIn(nodes, source) {
  return context => {
    for (let pos = (0, _language.syntaxTree)(context.state).resolve(context.pos, -1); pos; pos = pos.parent) if (nodes.indexOf(pos.name) > -1) return null;
...
class ActiveSource {
  ...
  update(tr, conf) {
    ...
    for (let effect of tr.effects) {
      ...
      /* Inactive */
      , false);else if (effect.is(setActiveEffect)) for (let active of effect.value) if (active.source == value.source) value = active;
    }
...
const startCompletionEffect = _state.StateEffect.define();

const closeCompletionEffect = _state.StateEffect.define();
...
const completionState = _state.StateField.define({
  ...
  provide: f => [_tooltip.showTooltip.computeN([f], state => state.field(f).tooltip), _view.EditorView.contentAttributes.from(f, state => state.attrs)]
});

const CompletionInteractMargin = 75; /// Returns a command that moves the completion selection forward or
...
const completionPlugin = _view.ViewPlugin.fromClass(class {
  ...
  update(update) {
    ...
    /* None */
        ...
        if (tr.annotation(_state.Transaction.userEvent) == "input") this.composing = 2
        /* Changed */
...
const nextSnippetField = moveField(1); /// Move to the previous snippet field, if available.
...
const snippetKeymap = _state.Facet.define({
  combine(maps) {
...
const addSnippetKeymap = _state.Prec.override(_view.keymap.compute([snippetKeymap], state => state.facet(snippetKeymap))); /// Create a completion from a snippet. Returns an object with the
...
const snippetPointerHandler = _view.EditorView.domEventHandlers({
...
const completeAnyWord = context => {
  ...
  * 2);
  ...
  for (let cur = context.state.doc.iterRange(start, end), pos = start; !cur.next().done;) {
    ...
    for (let i = 0;; i++) {
      if (i < value.length && cat(value[i]) == _state.CharCategory.Word) {
        if (start < 0) start = i;
      } else if (start > -1) {
...
function autocompletion(config = {}) {
  return [completionState, completionConfig.of(config), completionPlugin, completionKeymapExt, baseTheme, (0, _tooltip.tooltips)()];
...
const completionKeymapExt = _state.Prec.override(_view.keymap.computeN([completionConfig], state => state.facet(completionConfig).defaultKeymap ? [completionKeymap] : [])); /// Get the current completion status. When completions are available,
...
const SearchMargin = 50; /// Determines if the given range is block-commented in the given
/// state.
...
function findLineComment(token, lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    let line = lines[i],
        col = /^\s*/.exec(line.text)[0].length;
    let empty = skipped[line.number] = col == line.length;
    if (col < minCol && (!empty || minCol == 1e9 && i == lines.length - 1)) minCol = col;
    if (commented != false && (!empty || commented == null && i == lines.length - 1)) commented = line.text.slice(col, col + token.length) == token;
  }
...
function changeLineComment(option, ranges, state) {
  ...
  if (option != 2
  /* Uncomment */
  && lineRanges.some(c => !c.commented)) {
    let changes = [];

    for (let i = 0, lineRange; i < ranges.length; i++) if (!(lineRange = lineRanges[i]).commented) {
      for (let line of lines[i]) {
        if (!lineRange.skipped[line.number] || lines[i].length == 1) changes.push({
          from: line.from + lineRange.minCol,
          insert: tokens[i] + " "
        });
      }
    }
...
function getPos(view, event) {
  ...
  let col = off > MaxOff ? -1 : off == line.length ? absoluteColumn(view, event.clientX) : (0, _text.countColumn)(line.text.slice(0, offset - line.from), 0, view.state.tabSize);
  return {
    line: line.number,
    col,
    off
...
const lintState = _state.StateField.define({
  ...
  provide: f => [_panel.showPanel.computeN([f], s => {
    let {
      panel
    } = s.field(f);
    return panel ? [panel] : [];
...
function lintTooltip(view, pos, side) {
  ...
  return {
    ...
    create() {
      return {
        dom: (0, _crelt.default)("ul", found.map(d => renderDiagnostic(view, d, false)))
      };
    }
...
function renderDiagnostic(view, diagnostic, inPanel) {
  ...
  return (0, _crelt.default)("li", {
    class: (0, _view.themeClass)("diagnostic." + diagnostic.severity)
  }, (0, _crelt.default)("span", {
    class: (0, _view.themeClass)("diagnosticText")
    ...
    return (0, _crelt.default)("button", {
      class: (0, _view.themeClass)("diagnosticAction"),
      onclick: click,
      onmousedown: click
    }, nameElt);
  }), diagnostic.source && (0, _crelt.default)("div", {
    class: (0, _view.themeClass)("diagnosticSource")
  }, diagnostic.source));
}
...
class DiagnosticWidget extends _view.WidgetType {
  ...
  toDOM() {
    return (0, _crelt.default)("span", {
      class: (0, _view.themeClass)("lintPoint." + this.diagnostic.severity)
    });
  }
...
class LintPanel {
  constructor(view) {
    ...
    this.dom = (0, _crelt.default)("div", this.list, (0, _crelt.default)("button", {
      name: "close",
...
/// desired.
const basicSetup = [(0, _gutter.lineNumbers)(), (0, _view.highlightSpecialChars)(), (0, _history.history)(), (0, _fold.foldGutter)(), (0, _view.drawSelection)(), _state.EditorState.allowMultipleSelections.of(true), (0, _language.indentOnInput)(), _state.Prec.fallback(_highlight.defaultHighlightStyle), (0, _matchbrackets.bracketMatching)(), (0, _closebrackets.closeBrackets)(), (0, _autocomplete.autocompletion)(), (0, _rectangularSelection.rectangularSelection)(), (0, _view.highlightActiveLine)(), (0, _search.highlightSelectionMatches)(), _view.keymap.of([..._closebrackets.closeBracketsKeymap, ..._commands.defaultKeymap, ..._search.searchKeymap, ..._history.historyKeymap, ..._fold.foldKeymap, ..._comment.commentKeymap, ..._autocomplete.completionKeymap, ..._lint.lintKeymap])];
...
exports.Token = exports.Stack = exports.Parser = exports.ExternalTokenizer = void 0;
...
class Stack {
  /// @internal
  constructor( /// A group of values that the stack will share with all
  /// split instances
  ///@internal
  ...
  canShift(term) {
    for (let sim = new SimulatedStack(this);;) {
      let action = this.cx.parser.stateSlot(sim.top, 4
      /* DefaultReduce */
  ...
  get ruleStart() {
    for (let state = this.state, base = this.stack.length;;) {
      let force = this.cx.parser.stateSlot(state, 5
      /* ForcedReduce */
      );
      if (!(force & 65536
      /* ReduceFlag */
  ...
  forceReduce() {
    let reduce = this.cx.parser.stateSlot(this.state, 5
    /* ForcedReduce */
    );
    if ((reduce & 65536
    /* ReduceFlag */
  ...
  dialectEnabled(dialectID) {
    return this.cx.parser.dialect.flags[dialectID];
  }
...
function readToken(data, input, token, stack, group) {
  let state = 0,
      groupMask = 1 << group,
      dialect = stack.cx.parser.dialect;
...
function cutAt(tree, pos, side) {
  ...
  for (;;) {
    if (!(side < 0 ? cursor.childBefore(pos) : cursor.childAfter(pos))) for (;;) {
      if ((side < 0 ? cursor.to <= pos : cursor.from >= pos) && !cursor.type.isError) return side < 0 ? cursor.to - 1 : cursor.from + 1;
...
class Parser {
  ...
  constructor(spec) {
    ...
    for (let i = 0, l = this.states.length / 6
    /* Size */
  ...
  nextStates(state) {
    ...
    for (let term = 0; term < max; term++) {
      for (let pos = table[term + 1];;) {
        ...
        for (let end = pos + (groupTag >> 1); pos < end; pos++) if (table[pos] == state && result.indexOf(target) < 0) result.push(target);
...
},{"lezer-tree":"../node_modules/lezer-tree/dist/tree.es.js","process":"../node_modules/process/browser.js"}],"../node_modules/lezer-javascript/dist/index.es.js":[function(require,module,exports) {
"use strict";
...
const insertSemicolon = new _lezer.ExternalTokenizer((input, token, stack) => {
  let pos = token.start,
      next = input.get(pos);
...
const noSemicolon = new _lezer.ExternalTokenizer((input, token, stack) => {
  let pos = token.start,
      next = input.get(pos++);
...
const incdecToken = new _lezer.ExternalTokenizer((input, token, stack) => {
  let pos = token.start,
      next = input.get(pos);
...
const template = new _lezer.ExternalTokenizer((input, token) => {
  let pos = token.start,
      afterDollar = false;
...
const parser = _lezer.Parser.deserialize({
  ...
  maxTerm: 321,
  nodeProps: [[_lezer.NodeProp.group, -26, 7, 14, 16, 53, 174, 178, 182, 183, 185, 188, 191, 202, 204, 210, 212, 214, 216, 219, 225, 229, 231, 233, 235, 237, 239, 240, "Statement", -30, 11, 13, 23, 26, 27, 37, 38, 39, 40, 42, 47, 55, 63, 69, 70, 83, 84, 93, 94, 109, 112, 114, 115, 116, 117, 119, 120, 138, 139, 141, "Expression", -21, 22, 24, 28, 30, 142, 144, 146, 147, 149, 150, 151, 153, 154, 155, 157, 158, 159, 168, 170, 172, 173, "Type", -2, 74, 78, "ClassItem"], [_lezer.NodeProp.closedBy, 36, "]", 46, "}", 61, ")", 122, "JSXSelfCloseEndTag JSXEndTag", 136, "JSXEndTag"], [_lezer.NodeProp.openedBy, 41, "[", 45, "{", 60, "(", 121, "JSXStartTag", 131, "JSXStartTag JSXStartCloseTag"]],
  skippedNodes: [0, 4, 5],
  repeatNodeCount: 27,
...
exports.parser = parser;
},{"lezer":"../node_modules/lezer/dist/index.es.js"}],"../node_modules/@codemirror/lang-javascript/dist/index.js":[function(require,module,exports) {
"use strict";
...
var _lezerJavascript = require("lezer-javascript");
...
const snippets = [(0, _autocomplete.snippetCompletion)("function ${name}(${params}) {\n\t${}\n}", {
  label: "function",
  detail: "definition",
  type: "keyword"
}), (0, _autocomplete.snippetCompletion)("for (let ${index} = 0; ${index} < ${bound}; ${index}++) {\n\t${}\n}", {
  label: "for",
  detail: "loop",
  type: "keyword"
}), (0, _autocomplete.snippetCompletion)("for (let ${name} of ${collection}) {\n\t${}\n}", {
  label: "for",
  detail: "of loop",
  type: "keyword"
}), (0, _autocomplete.snippetCompletion)("try {\n\t${}\n} catch (${error}) {\n\t${}\n}", {
  label: "try",
  detail: "block",
  type: "keyword"
}), (0, _autocomplete.snippetCompletion)("class ${name} {\n\tconstructor(${params}) {\n\t\t${}\n\t}\n}", {
  label: "class",
  detail: "definition",
  type: "keyword"
}), (0, _autocomplete.snippetCompletion)("import {${names}} from \"${module}\"\n${}", {
  label: "import",
  detail: "named",
  type: "keyword"
}), (0, _autocomplete.snippetCompletion)("import ${name} from \"${module}\"\n${}", {
  label: "import",
  detail: "default",
  type: "keyword"
...
const javascriptLanguage = _language.LezerLanguage.define({
  parser: _lezerJavascript.parser.configure({
    props: [_language.indentNodeProp.add({
      IfStatement: (0, _language.continuedIndent)({
        except: /^\s*({|else\b)/
      }),
      TryStatement: (0, _language.continuedIndent)({
      ...
      TemplateString: _highlight.tags.special(_highlight.tags.string),
      ...
      "CallExpression/VariableName": _highlight.tags.function(_highlight.tags.variableName),
      VariableDefinition: _highlight.tags.definition(_highlight.tags.variableName),
      Label: _highlight.tags.labelName,
      PropertyName: _highlight.tags.propertyName,
      "CallExpression/MemberExpression/PropertyName": _highlight.tags.function(_highlight.tags.propertyName),
      "FunctionDeclaration/VariableDefinition": _highlight.tags.function(_highlight.tags.definition(_highlight.tags.variableName)),
      "ClassDeclaration/VariableDefinition": _highlight.tags.definition(_highlight.tags.className),
      PropertyNameDefinition: _highlight.tags.definition(_highlight.tags.propertyName),
      ...
      TypeDefinition: _highlight.tags.definition(_highlight.tags.typeName),
      "type enum interface implements namespace module declare": _highlight.tags.definitionKeyword,
      "abstract global privacy readonly": _highlight.tags.modifier,
...
},{"lezer-javascript":"../node_modules/lezer-javascript/dist/index.es.js","@codemirror/language":"../node_modules/@codemirror/language/dist/index.js","@codemirror/highlight":"../node_modules/@codemirror/highlight/dist/index.js","@codemirror/autocomplete":"../node_modules/@codemirror/autocomplete/dist/index.js"}],"index.js":[function(require,module,exports) {
"use strict";
...
var theme = _basicSetup.EditorView.theme({
  '&': {
    fontSize: '16pt'
  },
  '.cm-content': {
...
var initialState = _basicSetup.EditorState.create({
  doc: "function x() {} function x() {} function x() {} function x() {}\n  \n  const x=2  function x() {} function x() {} function x() {} function x() {}\n    \n  const x=2  \n  \n  function x() {} function x() {} function x() {} function x() {}\n    \n  const x=2  \n  \n  function x() {} function x() {} function x() {} function x() {}\n    \n  const x=2  \n  \n  function x() {} function x() {} function x() {} function x() {}\n    \n  const x=2  \n  \n  function x() {} function x() {} function x() {} function x() {}\n    \n  const x=2  \n  \n  \n  function x() {} function x() {} function x() {} function x() {}\n    \n  const x=2  \n  \n  function x() {} function x() {} function x() {} function x() {}\n    \n  const x=2  \n  \n  function x() {} function x() {} function x() {} function x() {}\n    \n  const x=2  \n  ",
  extensions: [_basicSetup.basicSetup, (0, _langJavascript.javascript)(), _basicSetup.EditorView.lineWrapping, theme]
});
...
window.view = view;
},{"@codemirror/basic-setup":"../node_modules/@codemirror/basic-setup/dist/index.js","@codemirror/lang-javascript":"../node_modules/@codemirror/lang-javascript/dist/index.js"}],"../node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
